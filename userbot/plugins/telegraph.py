...
from userbot.helpers.extras import delete_tornado as eod
...
tornado_mention = "[{TORNADO_NAME}](tg://user?id={thetornadoteam})"
lg_id = Config.TORNADOBOT_LOGGER
...
async def _(event):
    if event.fwd_from:
        return
    if Config.TORNADOBOT_LOGGER is None:
        await edit_or_reply(event, "You need to setup `TORNADOBOT_LOGGER` to use telegraph...", 7)
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **𝖥𝗂𝗅𝖾 𝗎𝗉𝗅𝗈𝖺𝖽𝖾𝖽 𝗍𝗈 : https://telegra.ph{}** \n✓ **𝖳𝗂𝗆𝖾 𝗍𝖺𝗄𝖾𝗇 :-** `{}` 𝗌𝖾𝖼𝗌 \n✓ **𝖡𝗒 :-** [{}](tg://user?id={})".format(
                        media_urls[0], (ms + ms_two), TORNADO_NAME, thetornadoteam
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            tornadoboy = f"https://telegra.ph/{response['path']}"
            await edit_or_reply(event, 
                  f"✓ **𝗉𝖺𝗌𝗍𝖾𝖽 𝗍𝗈** {tornadoboy} \n✓ **𝖳𝗂𝗆𝖾 𝗍𝖺𝗄𝖾𝗇 :-** `{ms}` 𝖲𝖾𝖼𝗌\n✓ **𝖡𝗒:-** [{TORNADO_NAME}](tg://user?id={thetornadoteam})", link_preview=True)
    else:
        await eod(event, 
