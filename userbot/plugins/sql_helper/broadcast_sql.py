...
class TornadoBroadcast(BASE):
    __tablename__ = "tornadobroadcast"
    ...
    def __repr__(self):
        return "<Tornado Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, TornadoBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


TornadoBroadcast.__table__.create(checkfirst=True)

TornadoBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with TornadoBROADCAST_INSERTION_LOCK:
        broadcast_group = TornadoBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with TornadoBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(TornadoBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with TornadoBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(TornadoBroadcast.keywoard)
            .filter(TornadoBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(TornadoBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(TornadoBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(TornadoBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(TornadoBroadcast).all()
        for x in all_groups:
