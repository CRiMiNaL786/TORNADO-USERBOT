# Ascii module by @kraken_the_badass for @tornadobot_official
# A over powerful bot
# I know u will kang...
# GTFO!! MOTHERFUCKER!!!!!!!!!!!
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    thetornadoteam = await edit_or_reply(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await thetornadoteam.edit("`Please unblock @asciiart_bot and try again`")
            return
        if response.text.startswith("Forward"):
            await thetornadoteam.edit(
...
async def _(event):
    ...
    if reply_message.sender.bot:
        await edit_or_reply(event, "Reply to actual users message.😒🤐")
        return
    thetornadoteam = await edit_or_reply(event, "`Processing`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await thetornadoteam.edit("Please unblock @Lines50Bot and try again")
            return
        await thetornadoteam.delete()
        await event.client.send_file(
            event.chat_id,
