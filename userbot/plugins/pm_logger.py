...
from tornadobot.utils import admin_cmd, register
...
TORNADOBOT_LOGGER = True
TORNADOBOT_LOGGER = Config.TORNADOBOT_LOGGER
...
async def log(log_text):
    ...
    if TORNADOBOT_LOGGER:
        if log_text.reply_to_msg_id:
            ...
            await reply_msg.forward_to(TORNADOBOT_LOGGER)
        elif log_text.pattern_match.group(1):
            user = f"#LOG / Chat ID: {log_text.chat_id}\n\n"
            textx = user + log_text.pattern_match.group(1)
            await bot.send_message(TORNADOBOT_LOGGER, textx)
        else:
...
async def monito_p_m_s(event):
    ...
    if Config.NC_LOG_P_M_S and not sender.bot:
        ...
        if chat.id not in NO_PM_LOG_USERS and chat.id != bot.uid:
            try:
                e = await bot.get_entity(int(Config.TORNADOBOT_LOGGER))
...
async def set_no_log_p_m(event):
    if Config.TORNADOBOT_LOGGER is not None:
