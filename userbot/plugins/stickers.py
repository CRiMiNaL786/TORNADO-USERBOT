...
tornadobot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_TornadoBot_{pack}"
        packnick = (
            f"{tornadobot} Vol.{pack}"
            if tornadobot
            else f"@{user.username}'s TornadoBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
