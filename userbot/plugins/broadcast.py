...
async def tornadobroadcast_send(event):
    if event.fwd_from:
        return
    tornadoinput_str = event.pattern_match.group(1)
    if not tornadoinput_str:
    ...
    keyword = tornadoinput_str.lower()
    ...
    tornadoevent = await edit_or_reply(
        event,
    ...
    await tornadoevent.edit(resultext)
    if BOTLOG:
...
async def tornadobroadcast_add(event):
    if event.fwd_from:
        return
    tornadoinput_str = event.pattern_match.group(1)
    if not tornadoinput_str:
...
async def tornadobroadcast_remove(event):
    if event.fwd_from:
        return
    tornadoinput_str = event.pattern_match.group(1)
    if not tornadoinput_str:
...
async def tornadobroadcast_list(event):
    if event.fwd_from:
        return
    tornadoinput_str = event.pattern_match.group(1)
    if not tornadoinput_str:
    ...
    await edit_or_reply(tornadoevent, finaloutput)
...
async def tornadobroadcast_delete(event):
    if event.fwd_from:
        return
    tornadoinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(tornadoinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {tornadoinput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(tornadoinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {tornadoinput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
