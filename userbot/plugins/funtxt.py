...
async def hmm(tornado):
    if tornado.fwd_from:
        return
    reactcat = nekos.textcat()
    await edit_or_reply(tornado, reactcat)
...
async def hmm(tornado):
    if tornado.fwd_from:
        return
    whytornado = nekos.why()
    await edit_or_reply(tornado, whytornado)
...
async def hmm(tornado):
    if tornado.fwd_from:
        return
    facttornado = nekos.fact()
    await edit_or_reply(tornado, facttornado)
