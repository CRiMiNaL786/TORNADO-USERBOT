...
# Help Pic feature added by @THETORNADOTEAM
...
def button(page, modules):
    ...
    for pairs in pairs[page]:
        buttons.append(
            [
                custom.Button.inline(f"{tornado_emoji} " + pair, data=f"Information[{page}]({pair})")
                for pair in pairs
            ]
        )
    ...
    buttons.append(
        [
            custom.Button.inline(
               f"◀️ ᏰᎯᏣᏦ {tornado_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{tornado_emoji} ❌ {tornado_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{tornado_emoji} ᏁᏋﾒᎿ ▶️", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
    return [max_pages, buttons]
    # Changing this line may give error in bot as i added some special cmds in TornadoBot channel to get this module work...
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@TornadoBot_Support":
            rev_text = query[::-1]
            veriler = button(0, sorted(CMD_HELP))
            result = await builder.article(
                f"Hey! Only use .help please",
                text=f"**Running TornadoBot**[⚡🔥]({tornado_help_pic})\n\n__Number of plugins installed__ :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=True,
            )
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN TORNADOBOT AND USE. © TornadoBot ™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF TornadoBot[⚡🔥]({tornado_help_pic})[.](https://t.me/TornadoBot_Support) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=True,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await event.edit(f"⚜️TornadoBot Menu Provider Is now Closed[⚜️]({tornado_help_pic})\n\n         **[© TornadoBot ™](t.me/TornadoBot_Support)**[⚡🔥]({tornado_help_pic})", 5, link_preview=True)
        else:
            tornado_alert = "HELLO THERE. PLEASE MAKE YOUR OWN TORNADOBOT AND USE. © TornadoBot ™"
            await event.answer(tornado_alert, cache_time=0, alert=True)
