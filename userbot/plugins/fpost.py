...
async def all_groups_id(tornado):
    tornadogroups = []
...
async def _(event):
    ...
    if Config.TORNADOBOT_LOGGER is None:
        await edit_or_reply(
            event,
            "Please set the required environment variable `TORNADOBOT_LOGGER` for this plugin to work",
        )
        return
    try:
        e = await event.client.get_entity(Config.TORNADOBOT_LOGGER)
