...
# porting to tornadobot by @kraken_the_badass...
...
async def get_users(event):
    ...
    if not sender.id == me.id:
        tornado = await edit_or_reply(event, "`processing...`")
    else:
        tornado = await edit_or_reply(event, "`processing...`")
    thetornadoteam = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await tornado.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await tornado.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(thetornadoteam.full_chat.id):
        try:
