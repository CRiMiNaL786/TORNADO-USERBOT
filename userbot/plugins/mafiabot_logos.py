...
#modifiy by @D3krish for Tornadobot
#If you kang this plugin please don't forget to give credit to Tornadobot and Hellbot. 
import os
import random
import time
...
async def _(tornadoevent):
    event = await edit_or_reply(tornadoevent, "`Processing.....`")
    fnt = await get_font_file(tornadoevent.client, "@D3VIL_FONTSS")
    if tornadoevent.reply_to_msg_id:
        rply = await tornadoevent.get_reply_message()
    ...
    text = tornadoevent.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "TornadoBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        tornadoevent.chat_id,
        file_name,
        caption=f"**мα∂ε вү  :** 『[{DEFAULTUSER}](tg://user?id={tornado})』\n\n",
    )
    await event.delete()
    try:
        os.remove(file_name)
