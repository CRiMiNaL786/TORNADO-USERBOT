...
from userbot import ALIVE_NAME, CUSTOM_PMPERMIT, TORNADO_ID
...
TORNADOPIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/cab5090d91c254ba90950.jpg"
)
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
...
USER_BOT_WARN_ZERO = "**You were spamming my sweet master's inbox, henceforth you have been blocked by my master's TornadoBot.**\n__Now GTFO, i'm busy__"
USER_BOT_NO_WARN = (
    "Hello, This is **TornadoBot Ultra Private Security Protocol⚠️**.\n"
    f"This is my master {DEFAULTUSER}'s Inbox\n"
    f"\n**{thetornadoteam}**\n\n"
    "To start a valid conversation\n🔱Register Your Request!🔱\nSend `/start` To Register Your Request\nHopefully u will get a reply🔥"
)

if Var.TORNADOBOT_LOGGER is not None:
        ...
        if len(APPROVED_PMs) > 4095:
            with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
                ...
                await event.client.send_file(
                    ...
                    force_document=True,
                    allow_cache=False,
                    caption="[TornadoBot]Current Approved PMs",
                    reply_to=event,
                )
                await event.delete()
        else:
    ...
    async def on_new_private_message(event):
        ...
        if Var.TORNADOBOT_LOGGER is None:
            return
    ...
    async def do_pm_permit_action(chat_id, event):
        ...
        r = await bot.send_file(
            event.chat_id, TORNADOPIC, caption=USER_BOT_NO_WARN, force_document=False
        )
        PM_WARNS[chat_id] += 1
        if chat_id in PREV_REPLY_MESSAGE:
            await PREV_REPLY_MESSAGE[chat_id].delete()
