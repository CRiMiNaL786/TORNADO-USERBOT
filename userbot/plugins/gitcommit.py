...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of tornadoBot`")
        return
    tornadobot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await tornadobot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await tornadobot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await tornadobot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, tornadobot)


async def git_commit(file_name, tornadobot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await tornadobot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await tornadobot.edit("Cannot Upload Plugin")
    else:
        return await tornadobot.edit("`Committed Suicide`")
