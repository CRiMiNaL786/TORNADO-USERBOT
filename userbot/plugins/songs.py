...
from tornadobot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(thetornadoteam):
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(tornado))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [TornadoBot](https://t.me/TornadoBot_Chit_Chat)",
            )
            await event.delete()
        except YouBlockedUserError:
