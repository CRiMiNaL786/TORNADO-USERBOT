...
from userbot.helpers.extras import delete_tornado, edit_or_reply as eor
...
TORNADO_NAME = str(ALIVE_NAME) if ALIVE_NAME else "Tornado User"
thetornadoteam = bot.uid
...
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):  
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit @TornadoBot_Support for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)        
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku App Name & API Key are filled correct. Visit @TornadoBot_Support for help.", link_preview=False)
   # event = await eor(dyno, "Downloading Logs...")
    tornado_data = app.get_log()
    await eor(dyno, tornado_data, deflink=True, linktext=f"**🗒️ Heroku Logs of 💯 lines. 🗒️**\n\n🌟 **Bot Of :** [{TORNADO_NAME}](tg://user?id={thetornadoteam})\n\n🚀** Pasted**  ")
