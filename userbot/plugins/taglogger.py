...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        thetornadoteam = await event.client.get_entity(event.sender_id)
        if thetornadoteam.bot or thetornadoteam.verified or thetornadoteam.support:
            return

        thetornadoteamm = f"[{get_display_name(thetornadoteam)}](tg://user?id={thetornadoteam.id})"
