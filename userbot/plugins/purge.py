...
from tornadobot.utils import admin_cmd, errors_handler, sudo_cmd, edit_or_reply
...
async def fastpurger(purg):
    ...
    if TORNADOBOT_LOGGER:
        await purg.client.send_message(
            TORNADOBOT_LOGGER, "Purge of " + str(count) + " messages done successfully."
        )
    await sleep(2)
    await done.delete()
...
async def selfdestruct(destroy):
    ...
    await smsg.delete()
    if TORNADOBOT_LOGGER:
        await destroy.client.send_message(TORNADOBOT_LOGGER, "sd query done successfully")
