...
async def promote(promt):
    ...
    tornadoevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await tornadoevent.edit("Promoted Successfully!")
    except BadRequestError:
        await tornadoevent.edit(NO_PERM)
        return
    if BOTLOG:
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    tornadoevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
    ...
    try:
        ...
        await tornadoevent.edit(NO_PERM)
        return
    await tornadoevent.edit("Demoted Successfully!")
    if BOTLOG:
...
async def ban(bon):
    ...
    if not user:
        return
    tornadoevent = await edit_or_reply(bon, "Banning this retard")
    try:
    ...
    try:
        ...
        await tornadoevent.edit("I ain't got msg deleting right. But still Banned!")
        return
    if reason:
        await tornadoevent.edit(f"{str(user.id)} is banned !!\nReason: {reason}")
    else:
        await tornadoevent.edit(f"{str(user.id)} is banned!")
    if BOTLOG:
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    tornadoevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await tornadoevent.edit("Unbanned Successfully!")
        if BOTLOG:
        ...
        await tornadoevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    tornadoevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await tornadoevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        ...
        await tornadoevent.edit(f"Kicked [{user.first_name}](tg://user?id={user.id})!")
    if BOTLOG:
