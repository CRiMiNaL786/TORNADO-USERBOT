...
async def _(event):
    ...
    if len(cmt) > 1:
        ...
        list_of_urls = get_inner_text(reply_message.message, reply_message.entities)
    converted_links = ""
...
async def get_direct_ip_specific_link(link: str):
    ...
    if "zippyshare.com" in link:
        async with aiohttp.ClientSession() as session:
            ...
            dl_url = {"url": base_url + eval(dl_url)}
        ...
        async with aiohttp.ClientSession(cookie_jar=aiohttp.CookieJar()) as session:
            ...
            if "location" in http_response.headers:
                ...
                http_response_two = await session.get(
                    warning_page_url, allow_redirects=False
                )
