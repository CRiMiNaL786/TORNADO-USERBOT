...
from tornadobot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor
from userbot.Config import Config
from . import *

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

thetornadoteam = tornadobot.uid

tornado = f"[{DEFAULTUSER}](tg://user?id={thetornadoteam})"

@tornadobot.on(admin_cmd(pattern="hvd$"))
@tornadobot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@tornadobot.on(admin_cmd(pattern="vday$"))
@tornadobot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
