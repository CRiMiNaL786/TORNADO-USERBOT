...
HIMANSHU = str(ALIVE_NAME) if ALIVE_NAME else "Tornado User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        tornadobot = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
        tornadobot = await edit_or_reply(ids, "`Ok! Gbaning this piece of shit....`")
    hum = await userbot.client.get_me()
    await tornadobot.edit(f"`🔥Global Ban Iz Cumin💦.... Wait and watch nigga🚶`")
    ...
    try:
        ...
        return await tornadobot.edit(f"**Something W3NT Wrong 🤔**")
    if user:
        if user.id == 1212368262:
            return await tornadobot.edit(
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await tornadobot.edit(f"Gbaning This retard🚶\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await tornadobot.edit(f"`Either reply to a user or gib me user id/name`")
    try:        
        if gmute(user.id) is False:
            return await tornadobot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await tornadobot.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        tornadobot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        tornadobot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await tornadobot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    try:
        ...
        return await tornadobot.edit("**Som3ting W3nt Wr0ng**")
    if user:
        if user.id == 1212368262:
            return await tornadobot.edit("**You need to grow some balls to gban / ungban my creator**")
        try:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await tornadobot.edit(f"Ok! Now Ungbaning this kiddo.\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await tornadobot.edit("**Reply to a user**")     
    try:
        if ungmute(user.id) is False:
            return await tornadobot.edit("**Error! I think User already ungbanned.**")
    except:
        pass
    return await tornadobot.edit(
