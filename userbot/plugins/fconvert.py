...
from tornadobot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(tornado):
    if tornado.fwd_from:
        return
    reply_to_id = tornado.message.id
    if tornado.reply_to_msg_id:
        reply_to_id = tornado.reply_to_msg_id
    event = await edit_or_reply(tornado, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await tornado.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await tornado.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    tornadoreply = await event.get_reply_message()
    if not tornadoreply or not tornadoreply.media or not tornadoreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if tornadoreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    tornadoevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, tornadoreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await tornadoevent.edit("`This file is not supported`")
            tornadoresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            tornadofile = Path(await event.client.download_media(tornadoresponse, "./temp/"))
            tornadogif = Path(await unzip(tornadofile))
            ...
            for files in (tornadogif, tornadofile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await tornadoevent.edit("Unblock @tgstogifbot")
            return
