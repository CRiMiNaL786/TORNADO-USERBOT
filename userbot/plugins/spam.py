...
async def bigspam(tornado):
    if not tornado.text[0].isalpha() and tornado.text[0] not in ("/", "#", "@", "!"):
        tornado_msg = tornado.text
        tornadobot_count = int(tornado_msg[9:13])
        tornado_spam = str(tornado.text[13:])
        for i in range(1, tornadobot_count):
            await tornado.respond(tornado_spam)
        await tornado.delete()
        if LOGGER:
            await tornado.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what tornadobot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
