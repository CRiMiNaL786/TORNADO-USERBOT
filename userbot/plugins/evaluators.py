...
from tornadobot.utils import admin_cmd, edit_or_reply, sudo_cmd, delete_tornado
from userbot import *
from userbot.cmdhelp import CmdHelp
from userbot.Config import Config

LOGGER = Config.TORNADOBOT_LOGGER
...
async def _(event):
    ...
    if not cmd:
        return await delete_tornado(event, "`What should i execute?..`")
    tornadoevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    tornadouser = await event.client.get_me()
    if tornadouser.username:
        curruser = tornadouser.username
    else:
        curruser = "tornadobot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    cmd = "".join(event.text.split(maxsplit=1)[1:])
    if not cmd:
        return await delete_tornado(event, "`What should i run ?..`")
    tornadoevent = await edit_or_reply(event, "`Running... Check Your Logger for result`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on TornadoBot server and shows details'
).add_command(
