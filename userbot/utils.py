...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("TornadoBot - Successfully imported " + shortname)
    else:
        import userbot.utils
        ...
        mod.delete_tornado = delete_tornado
        mod.media_type = media_type
        # support for tornadobot originals
        sys.modules["tornadobot.utils"] = userbot.utils
        sys.modules["tornadobot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                tornadoreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                tornadoreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(tornadoreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                tornadoreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_tornado(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        tornadoevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await tornadoevent.delete()
...
async def unsavegif(event, thetornadoteam):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=thetornadoteam.media.document.id,
                    access_hash=thetornadoteam.media.document.access_hash,
                    file_reference=thetornadoteam.media.document.file_reference,
                ),
                unsave=True,
            )
        )
