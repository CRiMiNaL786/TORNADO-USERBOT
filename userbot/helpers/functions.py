...
async def admin_groups(tornado):
    tornadogroups = []
    async for dialog in tornado.client.iter_dialogs():
        ...
        if (
            ...
            tornadogroups.append(entity.id)
    return tornadogroups
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            tornadofile = await event.client.download_media(tornadoresponse, "./temp")
            return await unzip(tornadofile)
...
async def iphonex(text):
    ...
    thetornadoteam = r.get("message")
    tornadourl = url(thetornadoteam)
    if not tornadourl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(thetornadoteam).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    tornado = await sticcers[0].click("me", hide_via=True)
    if tornado:
        await bot.send_file(int(chat_id), tornado, reply_to=reply_to_id)
